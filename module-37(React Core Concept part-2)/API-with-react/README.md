...
8. array of object 
9. map method 
10. vite config (for plugin react)
...
22. useEffect hook (different way to load data)
 const [players,setPlayers] = useState([])
    ...
    here first i have to take a useState for store the data from fetch .
    useState will take a default value as compare with fetch data (if array=array , object=object)
    then use useEffect it will take 2 parameter (callback function and dependency value . dependency will follow fetch value)
    inside the callback function fetch the data and set the value as useState 